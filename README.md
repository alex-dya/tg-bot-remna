...
При старте бот регистрирует меню команд в Telegram через `setMyCommands`:

- **Для обычных пользователей** (глобальный scope `default`) — только `/start`
- **Для каждого админа из `ADMIN_IDS`** индивидуально (scope `chat`) — полный набор: `/list`, `/add`, `/addlist`, `/del`, `/cancel`, `/help`, `/start`

Меню обновляется при каждом запуске бота. Если поменялся состав `ADMIN_IDS`,
перезапустите контейнер — старые админы потеряют расширенное меню только после
явного `deleteMyCommands` или ребута (Telegram кеширует scope `chat`).
...
| Команда  | Действие |
|----------|----------|
| `/start` | Получить ссылку подписки + ссылку на инструкцию (если задан `SUBSCRIPTION_INFO_URL`) |
